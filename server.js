...
if (!API_KEY || !API_SECRET) {
  console.error("Clés manquantes dans .env");
  process.exit(1);
}

console.log("Clés chargées OK");
...
app.post("/webhook", async (req, res) => {
  ...
  const body = {
    ...
    timestamp: timestamp,
    size: quantity.toString()  // ← On utilise TOUJOURS "size" pour la quantité de base asset
  };
...
const PORT = 3000;
app.listen(PORT, () => {
  console.log(`Serveur spot Pionex sur http://localhost:${PORT}`);
});